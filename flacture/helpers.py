...
def register_blueprints(app, package_name, package_path):
    ...
    for _, module_name, _ in pkgutil.iter_modules(package_path):
        _, api_dir_name = os.path.split(package_path[0])
        m = importlib.import_module('{0}.{1}.{2}'.format(package_name,
                                                         api_dir_name,
                                                         module_name))
        for item in dir(m):
            item = getattr(m, item)
...
class JsonSerializer(object):
    ...
    implements a :func:`__json__` method. The :func:`__json__` method is used
